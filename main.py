...
def raiz():
    return JSONResponse(
        status_code=200,
        content={"exitos": True, "mensaje": "Sistema de APIs"}
    )
...
def obtener_todos_articulos():
    if not articulos:
        return JSONResponse(
            status_code= 404, 
            content={
                "exito":False, "mensaje": "No se encontraron articulos"
            }
        )
    lista_articulos = [ArticuloRespuesta(id=articulo_id, **datos).model_dump() for articulo_id, datos in articulos.
    items()]
...
def obtener_articulo(articulo_id: int):
    if articulo_id not in articulos:
        return JSONResponse(
            status_code=404,
            content={"exito": False, "mensaje": "Articulo No encontrado"}
        )
...
def eliminar_articulo(articulo_id: int):
    ...
    return JSONResponse(
        ...
        content={
            "exito": True,
            "mensaje": f"Articulo {articulo_id} eliminado",
